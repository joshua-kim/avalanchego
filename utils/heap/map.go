...
var _ stdheap.Interface = (*indexedQueue[int, int])(nil)
...
func (m *Map[K, V]) Push(k K, v V) (V, bool) {
	if i, ok := m.queue.index[k]; ok {
		...
		stdheap.Fix(m.queue, i)
		return prev.v, true
	}

	stdheap.Push(m.queue, entry[K, V]{k: k, v: v})
...
func (m *Map[K, V]) Pop() (K, V, bool) {
	...
	popped := stdheap.Pop(m.queue).(entry[K, V])
...
func (m *Map[K, V]) Remove(i int) (K, V) {
	removed := stdheap.Remove(m.queue, i).(entry[K, V])
...
func (h *indexedQueue[K, V]) Len() int {
	return len(h.entries)
}

func (h *indexedQueue[K, V]) Less(i, j int) bool {
